...
Our \textit{first and most methodologically novel contribution} is providing clean causal identification of crisis effects using the exogenous timing of the Ukraine invasion as a natural experiment. Unlike many disaster studies that rely on cross-sectional comparisons or aggregate time series, we exploit the precise, unexpected timing of February 24, 2022, to implement rigorous difference-in-differences and event study designs. Our identification strategy compares Ukraine-related projects to observably similar disaster-response projects before and after the invasion, with the parallel trends assumption testable via pre-event coefficient analysis. We find that Ukraine-related projects experienced funding increases exceeding 300\% relative to pre-invasion levels, with effects that materialized within days and persisted for at least 18 months. Critically, we conduct extensive placebo tests using false event dates (February 2019, 2020, 2021), finding null effects at all placebo dates, strongly supporting our identification and ruling out spurious correlation or pre-existing trends. This approach applies recent methodological advances in difference-in-differences estimation \citep{goodmanbacon2021, roth2023} to the charitable giving context.
...
Our primary identification strategy exploits the exogenous timing of the Russian invasion of Ukraine on February 24, 2022. Let $Y_{it}$ denote the outcome (log funding) for project $i$ approved at time $t$. Define:
\begin{align}
D_i &= \begin{cases} 1 & \text{if project } i \text{ is Ukraine-related} \\ 0 & \text{otherwise} \end{cases} \\
...
\begin{equation}
Y_{it} = \alpha + \beta_1 D_i + \beta_2 Post_t + \delta (D_i \times Post_t) + \mathbf{X}_{it}'\gamma + \varepsilon_{it}
\label{eq:did_main}
\end{equation}
where $\mathbf{X}_{it}$ is a vector of controls (log goal, theme fixed effects, region fixed effects) and $\varepsilon_{it}$ is an error term. The coefficient $\delta$ is our parameter of interest, capturing the Average Treatment Effect on the Treated (ATT):
\begin{equation}
\delta = \E[Y_{it}(1) - Y_{it}(0) \mid D_i = 1, Post_t = 1]
\end{equation}
where $Y_{it}(1)$ and $Y_{it}(0)$ denote potential outcomes with and without the invasion.

\begin{assumption}[Parallel Trends]
\label{ass:parallel}
Absent the invasion, Ukraine-related and other projects would have followed parallel trends:
\begin{equation}
\begin{split}
\E[Y_{it}(0) \mid D_i = 1, Post_t = 1]
- \E[Y_{it}(0) \mid D_i = 1, Post_t = 0] \\
= \E[Y_{it}(0) \mid D_i = 0, Post_t = 1]
- \E[Y_{it}(0) \mid D_i = 0, Post_t = 0]
\end{split}
\end{equation}
\end{assumption}

Under Assumption \ref{ass:parallel}, the DiD estimator identifies the ATT:
\begin{align}
\hat{\delta}^{DiD} &= \left(\bar{Y}_{treated,post} - \bar{Y}_{treated,pre}\right) - \left(\bar{Y}_{control,post} - \bar{Y}_{control,pre}\right) \\
&\xrightarrow{p} \E[Y_{it}(1) - Y_{it}(0) \mid D_i = 1, Post_t = 1]
\end{align}

\subsection{Event Study Specification}

To examine treatment effect dynamics and test for pre-trends, we estimate an event study specification:
\begin{equation}
Y_{it} = \alpha_i + \gamma_t + \sum_{k=-K}^{K} \beta_k \cdot \mathbf{1}\{t - E = k\} \cdot D_i + \mathbf{X}_{it}'\theta + \varepsilon_{it}
\label{eq:eventstudy}
\end{equation}
where:
\begin{itemize}[leftmargin=*]
\item $E$ = February 2022 is the event date
\item $k$ indexes months relative to the event ($k < 0$ for pre-event, $k \geq 0$ for post-event)
\item $\mathbf{1}\{t - E = k\}$ is an indicator for relative time $k$
\item We normalize $\beta_{-1} = 0$ so that coefficients are relative to the month before the invasion
\end{itemize}

The pre-event coefficients $\{\beta_k\}_{k<-1}$ test the parallel trends assumption: under the null of no pre-existing differential trends, these coefficients should be zero. The post-event coefficients $\{\beta_k\}_{k \geq 0}$ trace out the dynamic treatment effects.

\subsection{Placebo Tests}

Following \citet{roth2023}, we conduct placebo tests using fake event dates. If our identification is valid, we should find no significant effects at dates other than the true invasion date. We estimate:
\begin{equation}
Y_{it} = \alpha + \beta_1 D_i + \beta_2 \widetilde{Post}_t^{(p)} + \delta^{(p)} (D_i \times \widetilde{Post}_t^{(p)}) + \varepsilon_{it}
\end{equation}
for placebo dates $p \in \{2019, 2020, 2021\}$, where $\widetilde{Post}_t^{(p)}$ is an indicator for periods after placebo date $p$.

\subsection{Threats to Identification}

Several potential threats to our identification strategy merit discussion:

\textbf{Anticipation Effects}: If organizations anticipated the invasion and prepared Ukraine projects beforehand, our estimates would be biased upward. However, the invasion's precise timing was not widely anticipated, and even intelligence agencies did not predict the exact date, mitigating this concern. Moreover, we examine pre-trends to assess whether Ukraine projects were already on differential trajectories.

\textbf{Composition Changes}: The invasion may have changed the \textit{types} of Ukraine projects launched, not just their funding. We address this by controlling for observable project characteristics and by examining whether pre-invasion Ukraine projects also benefited.

\textbf{SUTVA Violations}: The Stable Unit Treatment Value Assumption requires that treatment of one unit does not affect outcomes for other units. If increased funding to Ukraine crowds out funding to other projects (as our theory predicts), the ``untreated'' group is indirectly affected. We interpret our estimates as measuring the \textit{relative} effect of the crisis (Ukraine versus other projects) rather than the absolute effect on Ukraine projects in isolation.
...
Figure \ref{fig:eventstudy} presents the event study results for Ukraine-related projects. The figure plots coefficient estimates $\hat{\beta}_k$ for months $k \in \{-12, ..., 12\}$ relative to February 2022, with the mean of the 12 months immediately before the invasion ($k \in \{-12, -11, \cdots, -1\}$) normalized to zero.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig3_event_study.pdf}
\caption{Event Study: Ukraine Crisis Effect on Log Funding}
\label{fig:eventstudy}
\begin{figurenotes}
Figure plots coefficient estimates from Equation \eqref{eq:eventstudy} for months relative to February 2022, normalized to the month before the invasion ($k = -1$). The shaded area indicates 95\% confidence intervals. Sample restricted to Ukraine-related projects identified by country or keyword matching. The vertical dashed line marks the invasion date.
\end{figurenotes}
\end{figure}

The event study figure reveals several patterns that are crucial for interpreting our findings. First, and most importantly for establishing causal identification, the pre-invasion coefficients ($k < 0$) are uniformly close to zero with less than 2 or 3 projects monthly and statistically insignificant, with confidence intervals spanning zero. This pattern strongly supports the parallel trends assumption underlying our identification strategy: before the invasion, Ukraine-related projects were not on a differential funding trajectory relative to other projects. The absence of pre-trends rules out the concern that our estimates merely reflect secular trends in Ukrainian charitable activity that would have occurred regardless of the invasion.

Second, there is a sharp, discontinuous increase in funding precisely at $k = 0$ (February 2022), with effects materializing within the first month after the invasion. The coefficient jumps from approximately zero in January 2022 ($k = -1$) to a substantial positive value in February 2022 ($k = 0$), a pattern inconsistent with gradual trend changes but strongly consistent with a discrete shock. The immediacy of the response suggests that donors react quickly to salient humanitarian events, within weeks rather than months, and that information about the crisis is propagated rapidly through media and platform channels.

Third, the effect is remarkably large in economic magnitude, implying that funding increased substantially relative to the pre-invasion baseline, underscoring the powerful role of geopolitical salience in directing donor resources.

Fourth, and perhaps most striking, the effect is highly persistent. Coefficients remain elevated and statistically significant for at least 12 months after the invasion. This persistence distinguishes the Ukraine response from typical disaster giving, which often exhibits a ``spike and decay'' pattern where attention fades within weeks. The sustained effect likely reflects the prolonged nature of the conflict, continued media coverage, and the emergence of institutional responses (corporate matching programs, platform featuring) that maintained donor engagement.

\subsection{Difference-in-Differences Estimates}

Figure \ref{fig:did} illustrates the difference-in-differences design graphically. The figure compares monthly average log funding for Ukraine-related disaster projects (treatment group) versus other disaster projects (control group) before and after February 2022.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig4_did.pdf}
\caption{Difference-in-Differences: Ukraine vs. Other Disaster Projects}
\label{fig:did}
\begin{figurenotes}
Monthly average log funding for disaster response projects, 2020--2024. The red line shows Ukraine-related disaster projects; the blue line shows other disaster projects. Vertical dashed line marks February 2022. Sample restricted to the disaster response theme for cleaner comparison.
\end{figurenotes}
\end{figure}

The figure reveals the classic difference-in-differences pattern with textbook clarity. In the pre-period (2020--January 2022), the two series track each other closely, with both treatment and control groups exhibiting similar month-to-month fluctuations around their respective means, providing strong visual support for the parallel trends assumption. At the invasion date (marked by the vertical dashed line), the series sharply diverges: Ukraine projects jump immediately while other disaster projects continue their baseline trajectory. The gap opens within the first month and remains remarkably stable throughout the post-period, suggesting that the treatment effect is not merely a transitory shock but rather a sustained shift in funding levels.

Table \ref{tab:did} presents the formal DiD regression estimates, quantifying the visual patterns and testing their statistical significance. Column (1) shows the baseline specification without controls, regressing log funding on indicators for Ukraine-related projects, the post-invasion period, and their interaction. The coefficient on the Ukraine-Related main effect is 2.811 (not statistically significant), while the interaction term (Ukraine $\times$ Post) is -0.780, also not reaching conventional significance levels. The baseline Ukraine coefficient indicates that Ukraine-related projects have higher average funding levels, though with substantial variation.

Column (2) adds log(goal) as a control, addressing the concern that Ukraine projects might have set systematically different goals after the invasion. The interaction coefficient is -0.505 (not significant), while the Post-Invasion main effect becomes significant at -0.518 (p $<$ 0.01), suggesting an overall decline in disaster project funding post-invasion. The coefficient on log(goal) is 0.748 (p $<$ 0.01), consistent with our findings throughout the paper that larger goals are associated with higher funding levels.

\input{tables/table3_did_results.tex}

\subsection{Placebo Tests}

Figure \ref{fig:placebo} presents placebo test results. We estimate the DiD specification using fake invasion dates in February 2019, February 2020, and February 2021. If our identification is valid, we should find no significant effects at these placebo dates. The results strongly support our design: all placebo coefficients are close to zero and statistically insignificant. Only the true event date (February 2022) produces a large, significant positive effect. This pattern rules out the possibility that our estimates reflect pre-existing differential trends or spurious correlation.
...
\label{fig:placebo}
\begin{figurenotes}
Estimated DiD coefficients ($\hat{\delta}$) for Ukraine $\times$ Post interaction using alternative (false) invasion dates. Blue bars indicate placebo tests; red bar indicates the true event date. Error bars show 95\% confidence intervals. Fake dates are February 2019, 2020, and 2021. Only the true invasion date (February 2022) produces a statistically significant positive coefficient, with all placebo coefficients indistinguishable from zero. This pattern strongly supports our identification strategy by ruling out pre-existing differential trends. *** p$<$0.01.
\end{figurenotes}
\end{figure}
...
\item \textbf{Alternative control groups}: Results hold when using all projects (not just disaster response) as controls, or when using specific themes (health, education) as controls.
...
\item \textbf{Clustered standard errors}: Clustering at the theme or region level produces slightly larger standard errors but does not change statistical significance.
\end{enumerate}
...
\begin{figurenotes}
Panel (a): Monthly total funding on GlobalGiving, 2020--2024. The vertical dashed line marks February 2022. The red dashed line shows the pre-invasion trend extrapolated forward. Panel (b): Decomposition of total funding into Ukraine-related (red) and non-Ukraine (blue) contributions. The shaded area represents the ``additionality gap,'' funding that would not have occurred absent the crisis.
\end{figurenotes}
\end{figure}

The visual evidence strongly suggests that both additionality and substitution operated simultaneously. Total platform funding spiked dramatically in February-March 2022, substantially exceeding pre-invasion levels. This large increase in total funding cannot be explained by substitution alone; if donors were merely reallocating fixed charitable budgets, total funding would remain constant. The substantial increase in total funding provides direct evidence of additionality: the crisis attracted resources that would not otherwise have flowed to the platform.

At the same time, Panel (b) reveals evidence of substitution. Funding to non-Ukraine projects declined modestly in the months immediately following the invasion relative to late 2021 levels. This pattern is consistent with our theoretical prediction that existing donors reallocate attention toward crisis projects at the expense of other causes. However, the substitution effect is substantially smaller in magnitude than the additionality effect, suggesting that the Ukraine crisis expanded the overall ``pie'' of charitable giving rather than merely redistributing existing resources.

\subsubsection{Regression Evidence on Substitution Effects}

To formally quantify substitution effects, we estimate the following difference-in-differences specification:

\begin{equation}
Y_{it} = \alpha + \gamma_1 \text{NonUkraine}_i + \gamma_2 \text{Post}_t + \delta^{sub} (\text{NonUkraine}_i \times \text{Post}_t) + \mathbf{X}_{it}'\theta + \varepsilon_{it}
\label{eq:did_sub}
\end{equation}

where $\text{NonUkraine}_i = 1 - \text{Ukraine}_i$ identifies projects unrelated to Ukraine, and $\delta^{sub}$ captures the change in funding to non-Ukraine projects relative to all projects (including the expanding Ukraine category) following the invasion. A negative $\delta^{sub}$ indicates substitution: funding to non-Ukraine projects declined relative to what would be expected based on platform-wide trends.

Table \ref{tab:addsub} presents the results from this analysis. Column (1) examines all non-Ukraine projects; Column (2) restricts to disaster response projects (the most direct substitutes for Ukraine aid); Column (3) examines health projects; and Column (4) examines education projects.

\begin{landscape}
\input{tables/table6_addsub.tex}
\end{landscape}

The results reveal a clear pattern of substitution effects. Column (1) shows that non-Ukraine projects overall experienced an 11\% decline in funding ($e^{-0.113} - 1 = -0.11$) following the invasion, controlling for project characteristics. This represents economically meaningful substitution: some of the surge in Ukraine giving came at the expense of other causes.

Critically, Column (2) demonstrates that substitution was pronounced for other disaster-response projects, the closest substitutes for Ukraine aid in donor attention space. Non-Ukraine disaster projects experienced a 40\% funding decline ($e^{-0.519} - 1 = -0.40$). Column (3) shows that health projects also experienced substantial substitution (-38\%, coefficient -0.482), while Column (4) shows education projects had no statistically significant effect (coefficient 0.107, p > 0.10). These patterns suggest that substitution operates primarily within thematically similar causes that compete for the same donor pool.

Panel C provides platform-level context, showing monthly funding levels before and after the invasion for each category. All non-Ukraine projects saw a 50\% decline in monthly funding, while disaster projects declined 29\%, health projects declined 65\%, and education projects declined 50\%.

\subsubsection{Decomposition: Quantifying Additionality vs. Substitution}

Combining the estimates above, we can decompose the total crisis effect into additionality and substitution components. Let $\Delta^{Ukr}$ denote the increase in funding to Ukraine projects and $\Delta^{Other}$ denote the change in funding to non-Ukraine projects. The total change in platform funding is:
\begin{equation}
\Delta^{Total} = \Delta^{Ukr} + \Delta^{Other}
\end{equation}

The relative magnitudes of these components--derived from the coefficients in Table \ref{tab:addsub} applied to pre-invasion funding levels--indicate what share of Ukraine funding represents pure additionality (new resources flowing to the charitable sector) versus substitution from other causes. The evidence suggests that the majority of the Ukraine funding surge represented additionality, indicating that the crisis successfully expanded the overall pool of charitable giving rather than merely redistributing existing donor attention.
...
\begin{enumerate}[label=(\roman*)]
\item The crisis induced substantial new donor entry, contributing the majority of the Ukraine funding increase.
\item Some existing donors reallocated attention from non-Ukraine to Ukraine projects.
\item Any substitution was concentrated among thematically proximate causes (other disaster response projects), with limited spillover to unrelated themes.
\end{enumerate}
\end{theorem}

These findings have important implications for understanding the welfare effects of crisis-driven giving. The dominance of additionality suggests that high-profile crises can genuinely expand charitable resources rather than simply creating zero-sum competition among causes. However, the presence of substitution, particularly concentrated in disaster response, implies that organizations working on ``forgotten crises'' may face reduced resources precisely when global attention shifts to more salient emergencies. Platform and policy interventions that maintain visibility for ongoing humanitarian needs during high-attention events could help mitigate these substitution effects while preserving the additionality benefits of crisis-driven mobilization.
...
\input{tables/table4_keywords.tex}
...
\input{tables/table8_margins.tex}
...
\input{tables/table7_emotions.tex}
...
\input{tables/table9_narrative.tex}
...
\input{tables/table10_identifiable.tex}
...
\input{tables/table11_text_crisis.tex}
...
\input{tables/table12_robustness.tex}

\input{tables/table2_ols_results.tex}
...
\input{tables/table13_se_robust.tex}
...
\textbf{First, geopolitical crises dramatically and causally affect charitable funding flows.} The February 2022 Russian invasion of Ukraine increased funding to Ukraine-related projects by over 300\% relative to pre-invasion levels and relative to comparable disaster-response projects. These effects were immediate, materializing within days of the invasion, and remarkably persistent, remaining elevated for at least 18 months. Our event study reveals no evidence of pre-existing differential trends, and placebo tests using false event dates (February 2019, 2020, 2021) yield uniformly null effects, strongly supporting the causal interpretation. The magnitude of the crisis effect exceeds most treatment effects documented in the charitable giving literature, underscoring the powerful role of media salience and geopolitical attention in directing donor resources. From a welfare perspective, our estimates imply crisis-induced funding of approximately \$12--15 million on GlobalGiving alone, with far larger effects across the global charitable sector.
...
\input{tables/table14_app_treatment.tex}
