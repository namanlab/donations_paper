...
# ==============================================================================
# FIGURE 3: UKRAINE EVENT STUDY - AGGREGATE MONTHLY APPROACH
# ==============================================================================

cat("Creating Figure 3...\n")

# Use Date class for consistent matching (avoid timezone issues)
ukraine_event_date <- as.Date("2022-02-01")

# Convert approved_yearmonth to Date for matching
df <- df %>%
  mutate(approved_month_date = as.Date(approved_yearmonth))

# Create complete monthly sequence from 2020 to 2024 (full year) as Date
all_months_date <- seq(as.Date("2020-01-01"), as.Date("2024-12-01"), by = "month")

# Aggregate Ukraine-related projects by month
# CRITICAL: Include months with 0 Ukraine activity as zeros
ukraine_monthly <- df %>%
...
cat("Ukraine monthly data points:", nrow(ukraine_monthly), "\n")
cat("Total Ukraine funding:", sum(ukraine_monthly$total_funding), "\n")

# Complete with zeros for months with no Ukraine projects
...
cat("Complete monthly data points:", nrow(ukraine_monthly_complete), "\n")
cat("Max Ukraine funding:", max(ukraine_monthly_complete$total_funding), "\n")

# Create event study figure showing aggregate monthly funding
fig3 <- ukraine_monthly_complete %>%
  ...
  labs(
    ...
    subtitle = "Massive spike in February 2022; near-zero before invasion",
    x = NULL,
    y = "Total Monthly Funding ($M)",
    caption = "Note: Ukraine projects identified by country field or keyword matching in title/summary."
  )
...
# ==============================================================================
# FIGURE 4: DIFFERENCE-IN-DIFFERENCES - UKRAINE VS NON-UKRAINE
# ==============================================================================

cat("Creating Figure 4...\n")

# Aggregate by month and Ukraine status
did_monthly <- df %>%
  filter(approved_year >= 2020, approved_year <= 2024, !is.na(approved_month_date)) %>%
...
# Complete the data to include zeros for missing months
...
cat("DiD panel size:", nrow(did_complete), "\n")
cat("Ukraine pre-invasion mean funding:", mean(did_complete$total_funding[did_complete$is_ukraine & !did_complete$post]), "\n")
cat("Ukraine post-invasion mean funding:", mean(did_complete$total_funding[did_complete$is_ukraine & did_complete$post]), "\n")

# Main DiD figure - use log scale to see both series
fig4 <- did_complete %>%
  mutate(Group = ifelse(is_ukraine, "Ukraine-Related", "Non-Ukraine")) %>%
  ggplot(aes(x = approved_month_date, y = log_funding, color = Group)) +
  geom_vline(xintercept = ukraine_event_date, linetype = "dashed", color = "gray40", linewidth = 0.8) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 1.5) +
  scale_color_manual(values = c("Ukraine-Related" = pal_main[2], "Non-Ukraine" = pal_main[3])) +
  scale_x_date(date_labels = "%b %Y", date_breaks = "6 months") +
  annotate("text", x = ukraine_event_date - 30,
           y = 18, label = "Invasion\n(Feb 2022)", hjust = 1, size = 3, fontface = "italic") +
  labs(
    title = "Difference-in-Differences: Ukraine vs. Non-Ukraine Projects",
    subtitle = "Log of total monthly funding; vertical line marks February 2022",
    x = NULL,
    y = "Log(Total Monthly Funding + 1)",
    color = NULL,
    caption = "Note: Ukraine funding near zero pre-invasion (log scale compresses the spike)."
  ) +
  theme(legend.position = c(0.85, 0.2))

ggsave("figures/fig4_did.pdf", fig4, width = 8, height = 5, dpi = 300)
ggsave("figures/fig4_did.png", fig4, width = 8, height = 5, dpi = 300)
cat("Saved: fig4_did.pdf\n")

# Multi-panel figure with three outcomes: Funding, Donations, Projects
fig4_panel_funding <- did_complete %>%
  mutate(Group = ifelse(is_ukraine, "Ukraine", "Non-Ukraine")) %>%
  ggplot(aes(x = approved_month_date, y = total_funding / 1e6, color = Group)) +
  geom_vline(xintercept = ukraine_event_date, linetype = "dashed", color = "gray50") +
  geom_line(linewidth = 0.7) +
  geom_point(size = 1.2) +
  scale_color_manual(values = c("Ukraine" = pal_main[2], "Non-Ukraine" = pal_main[3])) +
  scale_x_date(date_labels = "%Y", date_breaks = "1 year") +
  scale_y_continuous(labels = dollar_format(suffix = "M")) +
  labs(title = "(a) Total Funding ($M)", x = NULL, y = NULL, color = NULL) +
  theme(legend.position = "none")

fig4_panel_donations <- did_complete %>%
  mutate(Group = ifelse(is_ukraine, "Ukraine", "Non-Ukraine")) %>%
  ggplot(aes(x = approved_month_date, y = total_donations / 1000, color = Group)) +
  geom_vline(xintercept = ukraine_event_date, linetype = "dashed", color = "gray50") +
  geom_line(linewidth = 0.7) +
  geom_point(size = 1.2) +
  scale_color_manual(values = c("Ukraine" = pal_main[2], "Non-Ukraine" = pal_main[3])) +
  scale_x_date(date_labels = "%Y", date_breaks = "1 year") +
  labs(title = "(b) Number of Donations (000s)", x = NULL, y = NULL, color = NULL) +
  theme(legend.position = "none")

fig4_panel_projects <- did_complete %>%
  mutate(Group = ifelse(is_ukraine, "Ukraine", "Non-Ukraine")) %>%
  ggplot(aes(x = approved_month_date, y = n_projects, color = Group)) +
  geom_vline(xintercept = ukraine_event_date, linetype = "dashed", color = "gray50") +
  geom_line(linewidth = 0.7) +
  geom_point(size = 1.2) +
  scale_color_manual(values = c("Ukraine" = pal_main[2], "Non-Ukraine" = pal_main[3])) +
  scale_x_date(date_labels = "%Y", date_breaks = "1 year") +
  labs(title = "(c) Number of Projects", x = NULL, y = NULL, color = NULL) +
  theme(legend.position = "bottom")

fig4_multi <- fig4_panel_funding + fig4_panel_donations + fig4_panel_projects +
  plot_layout(ncol = 3) +
  plot_annotation(
    title = "Ukraine vs. Non-Ukraine: Monthly Aggregates by Outcome",
    subtitle = "Vertical dashed line marks February 2022 invasion",
    caption = "Note: Ukraine surge visible across all three outcomes. Feb 2022 shows $74M funding, 269K donations to Ukraine projects."
  )

ggsave("figures/fig4_did_multipanel.pdf", fig4_multi, width = 12, height = 4, dpi = 300)
ggsave("figures/fig4_did_multipanel.png", fig4_multi, width = 12, height = 4, dpi = 300)
cat("Saved: fig4_did_multipanel.pdf\n")
...
# Table 3: DiD Results - AGGREGATE MONTHLY APPROACH
# Key insight: Ukraine projects were essentially 0 before Feb 2022
# We use aggregate monthly data with zeros properly included

cat("Creating Table 3 (DiD with aggregate monthly data)...\n")

# The did_complete panel was already created in Figure 4 section
# It has: ukraine, ukraine_post, post, total_funding, total_donations, n_projects, log_funding, log_donations

# Run DiD regressions on monthly panel
did_funding_m1 <- lm(total_funding ~ ukraine + post + ukraine_post, data = did_complete)
did_logfunding_m1 <- lm(log_funding ~ ukraine + post + ukraine_post, data = did_complete)
did_donations_m1 <- lm(total_donations ~ ukraine + post + ukraine_post, data = did_complete)
did_projects_m1 <- lm(n_projects ~ ukraine + post + ukraine_post, data = did_complete)

# Print summaries for debugging
cat("\n=== DiD Regression: Total Funding ===\n")
print(summary(did_funding_m1))
cat("\n=== DiD Regression: Log Funding ===\n")
print(summary(did_logfunding_m1))
cat("\n=== DiD Regression: Total Donations ===\n")
print(summary(did_donations_m1))
cat("\n=== DiD Regression: N Projects ===\n")
print(summary(did_projects_m1))

# Helper function for formatting coefficients
format_coef_new <- function(model, term) {
  coefs <- summary(model)$coefficients
  if (!(term %in% rownames(coefs))) return(list(est = "--", se = "--"))
  ...
  if (abs(est) >= 1e6) {
    est_str <- sprintf("%.2fM%s", est/1e6, stars)
    se_str <- sprintf("(%.2fM)", se/1e6)
  } else if (abs(est) >= 1000) {
    est_str <- sprintf("%sK%s", format(round(est/1000, 1), nsmall=1), stars)
    se_str <- sprintf("(%sK)", format(round(se/1000, 1), nsmall=1))
  } else if (abs(est) >= 1) {
    est_str <- sprintf("%.2f%s", est, stars)
    se_str <- sprintf("(%.2f)", se)
  } else {
    est_str <- sprintf("%.3f%s", est, stars)
    se_str <- sprintf("(%.3f)", se)
  }
  return(list(est = est_str, se = se_str))
}

# Extract all coefficients
c_ukr_post_fund <- format_coef_new(did_funding_m1, "ukraine_post")
c_ukr_post_log <- format_coef_new(did_logfunding_m1, "ukraine_post")
c_ukr_post_don <- format_coef_new(did_donations_m1, "ukraine_post")
c_ukr_post_proj <- format_coef_new(did_projects_m1, "ukraine_post")

c_ukr_fund <- format_coef_new(did_funding_m1, "ukraine")
c_ukr_log <- format_coef_new(did_logfunding_m1, "ukraine")
c_ukr_don <- format_coef_new(did_donations_m1, "ukraine")
c_ukr_proj <- format_coef_new(did_projects_m1, "ukraine")

c_post_fund <- format_coef_new(did_funding_m1, "postTRUE")
c_post_log <- format_coef_new(did_logfunding_m1, "postTRUE")
c_post_don <- format_coef_new(did_donations_m1, "postTRUE")
c_post_proj <- format_coef_new(did_projects_m1, "postTRUE")

# Pre/post means for context
pre_ukr_fund <- did_complete %>% filter(is_ukraine, !post) %>% summarise(m = mean(total_funding)) %>% pull(m)
post_ukr_fund <- did_complete %>% filter(is_ukraine, post) %>% summarise(m = mean(total_funding)) %>% pull(m)
pre_nonukr_fund <- did_complete %>% filter(!is_ukraine, !post) %>% summarise(m = mean(total_funding)) %>% pull(m)
post_nonukr_fund <- did_complete %>% filter(!is_ukraine, post) %>% summarise(m = mean(total_funding)) %>% pull(m)

cat("\nMeans for context:\n")
cat("Pre-invasion Ukraine mean funding: $", format(round(pre_ukr_fund), big.mark=","), "\n")
cat("Post-invasion Ukraine mean funding: $", format(round(post_ukr_fund), big.mark=","), "\n")
cat("Pre-invasion Non-Ukraine mean funding: $", format(round(pre_nonukr_fund), big.mark=","), "\n")
cat("Post-invasion Non-Ukraine mean funding: $", format(round(post_nonukr_fund), big.mark=","), "\n")
...
did_latex <- paste0(
  ...
  "\\midrule\n",
  sprintf("Ukraine $\\times$ Post & %s & %s & %s & %s \\\\\n",
          c_ukr_post_fund$est, c_ukr_post_log$est, c_ukr_post_don$est, c_ukr_post_proj$est),
  sprintf("& %s & %s & %s & %s \\\\\n",
          c_ukr_post_fund$se, c_ukr_post_log$se, c_ukr_post_don$se, c_ukr_post_proj$se),
  "\\addlinespace\n",
  sprintf("Ukraine (main effect) & %s & %s & %s & %s \\\\\n",
          c_ukr_fund$est, c_ukr_log$est, c_ukr_don$est, c_ukr_proj$est),
  sprintf("& %s & %s & %s & %s \\\\\n",
          c_ukr_fund$se, c_ukr_log$se, c_ukr_don$se, c_ukr_proj$se),
  "\\addlinespace\n",
  sprintf("Post-Invasion & %s & %s & %s & %s \\\\\n",
          c_post_fund$est, c_post_log$est, c_post_don$est, c_post_proj$est),
  sprintf("& %s & %s & %s & %s \\\\\n",
          c_post_fund$se, c_post_log$se, c_post_don$se, c_post_proj$se),
  "\\midrule\n",
  sprintf("Observations & %d & %d & %d & %d \\\\\n",
          nrow(did_complete), nrow(did_complete), nrow(did_complete), nrow(did_complete)),
  sprintf("R-squared & %.3f & %.3f & %.3f & %.3f \\\\\n",
          summary(did_funding_m1)$r.squared, summary(did_logfunding_m1)$r.squared,
          summary(did_donations_m1)$r.squared, summary(did_projects_m1)$r.squared),
  "\\addlinespace\n",
  sprintf("Pre-invasion Ukraine mean & \\$%s & --- & %s & %.1f \\\\\n",
          format(round(pre_ukr_fund), big.mark=","),
          format(round(did_complete %>% filter(is_ukraine, !post) %>% summarise(m=mean(total_donations)) %>% pull(m)), big.mark=","),
          did_complete %>% filter(is_ukraine, !post) %>% summarise(m=mean(n_projects)) %>% pull(m)),
  sprintf("Post-invasion Ukraine mean & \\$%s & --- & %s & %.1f \\\\\n",
          format(round(post_ukr_fund), big.mark=","),
          format(round(did_complete %>% filter(is_ukraine, post) %>% summarise(m=mean(total_donations)) %>% pull(m)), big.mark=","),
          did_complete %>% filter(is_ukraine, post) %>% summarise(m=mean(n_projects)) %>% pull(m)),
  "\\bottomrule\n",
  "\\end{tabular}\n",
...
# Monthly funding by Ukraine vs Non-Ukraine (using Date class)
monthly_ukraine_nonukraine <- df %>%
  filter(!is.na(approved_month_date), approved_year >= 2020, approved_year <= 2024) %>%
  mutate(
    category = ifelse(is_ukraine, "Ukraine-Related", "Non-Ukraine")
  ) %>%
  group_by(approved_month_date, category) %>%
  summarise(
    total_funding = sum(funding, na.rm = TRUE),
    n_projects = n(),
    .groups = "drop"
...
# Complete with zeros for Ukraine pre-invasion
monthly_ukraine_complete_fig12 <- expand_grid(
  approved_month_date = all_months_date,
  category = c("Ukraine-Related", "Non-Ukraine")
) %>%
  left_join(monthly_ukraine_nonukraine, by = c("approved_month_date", "category")) %>%
  mutate(
    total_funding = replace_na(total_funding, 0),
    n_projects = replace_na(n_projects, 0)
  )
...
pre_data <- monthly_total %>%
  filter(approved_month_date < ukraine_event_date)
trend_model <- lm(total_funding ~ as.numeric(approved_month_date), data = pre_data)

monthly_total <- monthly_total %>%
  mutate(
    counterfactual = predict(trend_model, newdata = .),
    additionality = ifelse(approved_month_date >= ukraine_event_date,
                           pmax(0, total_funding - counterfactual), NA)
  )

# Panel A: Total funding with counterfactual
p12a <- monthly_total %>%
  ggplot(aes(x = approved_month_date)) +
  geom_ribbon(aes(ymin = counterfactual / 1e6, ymax = total_funding / 1e6),
              data = . %>% filter(approved_month_date >= ukraine_event_date),
  ...
  geom_vline(xintercept = ukraine_event_date, linetype = "dotted", color = "gray40") +
  scale_x_date(date_labels = "%Y", date_breaks = "1 year") +
  scale_y_continuous(labels = dollar_format(suffix = "M")) +
  annotate("text", x = ukraine_event_date + 120, y = max(monthly_total$total_funding / 1e6, na.rm = TRUE) * 0.9,
...
# Panel B: Decomposition by Ukraine vs Non-Ukraine
p12b <- monthly_ukraine_complete_fig12 %>%
  ggplot(aes(x = approved_month_date, y = total_funding / 1e6, fill = category)) +
  ...
  scale_y_continuous(labels = dollar_format(suffix = "M")) +
  labs(
    title = "(b) Funding by Project Type",
    subtitle = "Ukraine-related vs. Non-Ukraine projects",
...
# Did non-Ukraine projects see reduced funding after the invasion?
# This tests whether the Ukraine surge came at the expense of other causes

# Prepare data for substitution analysis
addsub_base <- df %>%
...
# Different samples for substitution analysis
addsub_all <- addsub_base %>% filter(!is_ukraine, !is.na(log_goal))  # All non-Ukraine
addsub_disaster <- addsub_base %>% filter(!is_ukraine, is_disaster, !is.na(log_goal))  # Non-Ukraine disaster (within-theme substitution)
addsub_health <- addsub_base %>% filter(is_health, !is.na(log_goal))
addsub_education <- addsub_base %>% filter(is_education, !is.na(log_goal))
...
# Run regressions for Panel A: Log(Funding) - Non-Ukraine projects by category
...
# Panel A coefficients (non-Ukraine projects - testing substitution)
...
# Generate LaTeX table - Substitution analysis on non-Ukraine projects
addsub_latex <- paste0(
  "\\begin{table}[htbp]\n",
  "\\centering\n",
